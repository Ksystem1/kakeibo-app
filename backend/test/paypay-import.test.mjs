...
test("buildPayPayImportPlan: 日付のみ（時刻なし）でも解析できる", () => {
  const csv = [
    SAMPLE_HEADER,
    "2026/04/22,100,-,-,-,-,-,支払い,七福亭,PayPay残高,-,-,date-only-1",
  ].join("\n");
  const plan = buildPayPayImportPlan(csv, { combineSameTimePayments: false });
  assert.equal(plan.ok, true);
  assert.equal(plan.records.length, 1);
  assert.equal(plan.records[0].txDate, "2026-04-22");
  assert.equal(plan.records[0].txSecond, "2026-04-22 00:00:00");
});
