/**
 * RDS に db/migration_v46_monitor_recruitment_capacity.sql を適用する。
 * （npm run db:migrate-v46 は receipt_learning_catalog.admin_note 用であり、この SQL とは別）
 * 実行: cd backend && npm run db:migrate-v46-monitor-capacity
 */
...
const migrationPath = path.resolve(
  ...
  "..",
  "db",
  "migration_v46_monitor_recruitment_capacity.sql",
);
...
try {
  ...
  console.log("migration_v46_monitor_recruitment_capacity.sql の適用が完了しました。");
} finally {
  await conn.end();
}
